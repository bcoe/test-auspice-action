...
describe("smoke test rendering of paths in urls.txt", () => {
  for (const testCase of testCases) {
    ...
    it(`it appropriately renders ${testCase.path}`, async () => {
      const resp = await page.goto(`${BASE_URL}${testCase.path}`, { waitUntil: 'networkidle2' });
      ...
      const snapshotText = cleanSnapshotText(JSON.stringify(await client.send('Page.captureSnapshot')));
...
// MIME HTML might have lines terminated with =\r\n, combine these lines
// back together:
function cleanSnapshotText(snapshotText) {
  let snapshotTextNowNewlines = snapshotText;
  while (snapshotTextNowNewlines.match(/=\\r\\n/)) {
    snapshotTextNowNewlines = snapshotTextNowNewlines.replace(/=\\r\\n/g, '');
  }
  return snapshotTextNowNewlines;
}
